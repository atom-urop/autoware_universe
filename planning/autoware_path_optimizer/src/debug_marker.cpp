  ...
  const std::vector<ReferencePoint> & ref_points)
{
  ...
  for (size_t i = 0; i < ref_points.size(); i++) {
    ...
    marker.text =
      std::to_string(ref_points.at(i).curvature) + " \n" +
      "front: " + std::to_string(ref_points.at(i).optimized_front_steer) + " \n" +
      "rear: " + std::to_string(ref_points.at(i).optimized_rear_steer);
